...
function decodeConfig(raw) {
    const value = (raw ?? {});
    const preset = typeof value.preset === "string" && value.preset in PRESETS ? value.preset : "opencode";
    return {
        preset,
...
function extensionDirectories(prefixes) {
    const matches = [];
...
function bundledExtensionCommand(preset) {
...
function detectedEditorSurface(preset) {
    const definitions = {
    ...
    if (definition && extensionDirectories(definition.prefixes).length)
    ...
    if (preset === "gemini" && existsSync(join(homedir(), ".antigravity")))
        return "Google Antigravity editor";
    return null;
}
function resolveWindowsPath(cli) {
    try {
        ...
        if (cmd)
            return { command: process.env.ComSpec ?? "cmd.exe", prefix: ["/d", "/s", "/c", cmd] };
    }
    catch { }
    return null;
}
function resolveCommand(cli, preset) {
    if (process.platform !== "win32" || /[\\/]/.test(cli))
    ...
    if (preset === "gemini") {
        ...
        if (existsSync(nativeAgy))
            return { command: nativeAgy, prefix: [] };
    }
...
function commandArgs(config, turn) {
...
function textFromEvent(value) {
    if (!value || typeof value !== "object")
        return {};
    const sessionId = value.session_id ?? value.sessionId ?? value.sessionID ?? value.session?.id;
    if (typeof value.response === "string")
        return { text: value.response, final: true, sessionId };
    if (typeof value.result === "string")
        return { text: value.result, final: true, sessionId };
...
export const ExternalCliDriver = {
    ...
    async create(input) {
        ...
        return {
            ...
            adapter: {
                ...
                stopAll: async () => { for (const turn of active.values())
                    turn.stop(); },
