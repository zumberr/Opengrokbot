...
import { MAX_BOTS, Store } from "./store.js";
...
const collaborationTurns = new Map();
...
const smartRuns = new Map();
let availabilityCache = null;
async function availableInstances() {
    if (availabilityCache && Date.now() - availabilityCache.at < 30_000)
...
async function smartCandidates(role) {
    ...
    setTimeout(() => {
        void startTurn(coordinator.id, synthesis, {
            ...
            appendAndBroadcast(coordinator.threadId, {
                role: "bot",
                kind: "activity",
                tool: { name: `team synthesis failed: ${error.message.slice(0, 140)}`, ok: false },
            });
        });
    }, 0);
}
    ...
    switch (event.type) {
        ...
        case "item.completed":
            if (event.itemType === "assistant_text") {
                ...
                if (collaboration)
                    collaboration.output.push(event.text);
            }
        ...
        case "runtime.error": {
            ...
            if (smart)
                smart.failures.push(event.message);
            else
            ...
            if (!event.ok && smart && smart.index + 1 < smart.candidates.length) {
                ...
                void smart.launch().catch((error) => {
                    ...
                    pushMessage({
                        role: "bot",
                        kind: "activity",
                        tool: { name: `all smart routes failed: ${error.message.slice(0, 130)}`, ok: false },
                    });
...
async function startTurn(botId, text, options = {}) {
    ...
    const candidates = bot.routingMode === "manual"
    ...
    if (!candidates.length || !initialInstance) {
        throw Object.assign(new Error(bot.routingMode === "manual"
            ? `provider instance "${bot.modelSelection.instanceId}" is unavailable — pick another model in settings`
            : `no providers are available for the ${SMART_POLICIES[role].label} smart route`), { status: 409 });
    }
    ...
    void (async () => {
        try {
            ...
            const launch = async () => {
                ...
                if (!instance)
                    throw new Error(`${candidate.label} became unavailable`);
                ...
                try {
                    ...
                    if (current && current.index + 1 < current.candidates.length) {
                        current.failures.push(error.message);
...
const server = createServer(async (req, res) => {
    ...
    try {
        ...
        if (m && method === "PATCH") {
            ...
            const patch = {};
            for (const key of ["name", "title", "description", "notifications", "modelSelection", "routingMode", "smartRole", "unread", "computer", "color", "mascotExpression", "pinned", "hidden"]) {
        ...
        if (m && method === "POST") {
            ...
            if (!coordinator)
                return json(res, 404, { error: "no such coordinator bot" });
            const body = await readBody(req);
            const text = String(body.text ?? "").trim();
            if (!text)
                return json(res, 400, { error: "text required" });
            const requested = Array.isArray(body.botIds)
                ? body.botIds.map((id) => String(id))
                : [];
            const peerIds = [...new Set(requested)].filter((id) => id !== coordinator.id);
            if (!peerIds.length)
                return json(res, 400, { error: "choose at least one collaborator" });
            if (peerIds.length > 7)
                return json(res, 400, { error: "a team run supports up to 7 collaborators" });
            const participants = [coordinator.id, ...peerIds].map((id) => store.bot(id));
            if (participants.some((bot) => !bot))
                return json(res, 404, { error: "one or more collaborators no longer exist" });
            ...
            for (const participant of participants) {
                ...
                void startTurn(participant.id, prompt, {
                    ...
                    run.responses.set(participant.id, `Failed to respond: ${error.message}`);
                    run.pending.delete(participant.id);
                    if (!run.pending.size)
                        completeCollaborationDraft(participant.id, participant.threadId);
                });
            }
