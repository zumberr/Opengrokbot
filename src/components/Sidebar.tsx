...
function BotListItem({ bot, onMenu }: { bot: Bot; onMenu: (menu: MenuState) => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative flex w-full items-center gap-2.5 rounded-xl border px-2.5 py-2 text-left transition",
        selected
          ? "border-accent/25 bg-accent/8 shadow-[inset_3px_0_0_var(--color-accent)]"
          : "border-transparent hover:border-hairline hover:bg-raised/45",
      )}
    >
      <div className="relative">
        <MausAvatar color={bot.color} expression={expressionForBot(bot)} size={38} />
        <span
          className={cn(
            "absolute -bottom-0.5 -right-0.5 size-2.5 rounded-full border-2 border-panel",
            bot.busy ? "status-pulse bg-accent" : "bg-success",
          )}
        />
      </div>
...
export function Sidebar() {
  ...
  const atLimit = state.bots.length >= state.maxBots;

  const visibleBots = state.bots
    .filter((b) => !b.hidden)
    .filter((b) => `${b.name} ${b.title} ${b.description}`.toLowerCase().includes(query.trim().toLowerCase()))
    .sort((a, b) => Number(b.pinned ?? false) - Number(a.pinned ?? false));

  return (
    <aside className="sidebar-grid flex h-full w-[304px] shrink-0 flex-col border-r border-hairline bg-panel">
      {/* Titlebar: real traffic lights in Electron, faux ones in the browser */}
      <div
        className="relative flex items-center justify-between px-4 pb-2 pt-3.5"
      ...
      >
        ...
        <div className="pointer-events-none absolute left-1/2 flex -translate-x-1/2 items-center gap-2">
          <span className="grid size-6 place-items-center rounded-lg border border-accent/25 bg-accent/10 text-accent">
            <Sparkles size={13} />
          </span>
          <span className="font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-ink">OpenGrok</span>
        </div>
        <button
          onClick={() => { track("bot_created"); dispatch({ type: "newBot" }); }}
          disabled={atLimit}
          className="rounded-lg border border-hairline bg-inset p-1.5 text-ink-secondary transition hover:border-accent/40 hover:text-accent disabled:cursor-not-allowed disabled:opacity-35"
          style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
          title={atLimit ? `Limit reached (${state.maxBots} bots)` : "New bot"}
        >
      ...
      <div className="px-3 pb-2 pt-2">
        <div className="flex items-center gap-2 rounded-xl border border-hairline bg-inset/80 px-3 py-2.5 focus-within:border-accent/40">
        ...
        <div className="flex items-center justify-between px-1 pb-1 pt-3">
          <span className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-ink-secondary">Agent fleet</span>
          <span className="rounded-full border border-hairline bg-inset px-2 py-0.5 font-mono text-[10px] text-ink-secondary">
            {state.bots.length}/{state.maxBots}
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2">
        <div className="flex flex-col gap-0.5">
          ...
          {!visibleBots.length && (
            <div className="mx-2 mt-8 rounded-2xl border border-dashed border-hairline px-4 py-6 text-center text-[12px] text-ink-secondary">
      ...
      {/* Footer */}
      <div className="border-t border-hairline/70 bg-inset/40 px-3 pb-3 pt-2">
        <button
